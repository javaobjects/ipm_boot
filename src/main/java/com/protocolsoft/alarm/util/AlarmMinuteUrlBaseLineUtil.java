 ...
 *类名: AlarmMinuteUrlBaseLineUtil.java
...
/**
 * 
 * @ClassName: AlarmMinuteUrlBaseLineUtil
 * @Description: url业务智能告警工具类
 * @author chensq
 */
public class AlarmMinuteUrlBaseLineUtil {

    /**
     * 告警内容list
     */
    public static List<Map<String, AlarmCheckCountBean>> checkBaseLineAlarmMinuteUrlList;
    ...
    public static void checkBaseLineInfo(
        ...
        UrlRrdService urlRrdService=alarmBaseLineUseDaoBean.getUrlRrdService();
    ...
    public static void setToCollection(
        ...
        //初始化
        if (checkBaseLineAlarmMinuteUrlList == null) {
            checkBaseLineAlarmMinuteUrlList=new CopyOnWriteArrayList<Map<String, AlarmCheckCountBean>>();
        }
        
        //验证以前是否存在过该key
        boolean beforeIsHave=false; //false未存在过  true存在过
        if (checkBaseLineAlarmMinuteUrlList!=null && checkBaseLineAlarmMinuteUrlList.size()>0) {
            for (int x=0; x<checkBaseLineAlarmMinuteUrlList.size(); x++) {
                Map<String, AlarmCheckCountBean> tempItemsMap=checkBaseLineAlarmMinuteUrlList.get(x);
                //迭代map
        ...
        if (!beforeIsHave) {//未设置
            ...
            checkBaseLineAlarmMinuteUrlList.add(checkAlarmItemMap);
        } else {//已设置
                    ...
                    while (its.hasNext()) {
                        ...
                        if (itemKey.equals(key)) {
                            ...
                            if (checkResult==1) {// 1为继续
                                ...
                                checkBaseLineAlarmMinuteUrlList.remove(tempItemsMap);
//                                its.remove();
                            } else {//3为符合告警条件
