                ...
                if (mouse.target != false && mouse.clone != false) {
                    ...
                    if ($.contains(daddy[0], mouse.target[0])) {
                        mouse.clone.animate({
                            ...
                            //top: mouse.target.offset().top  - bTop,
                            //left: mouse.target.offset().left  - bLeft
            ...
            daddy.find(jq).on('mousedown touchstart', function(e) {
                if($("a[data-drawer='lockStatus']").children("span").text() != "锁定"){
                    if (mouse.target == false && e.which == 1 && active == true) {
                        ...
                        //var _olef = $("#sidebar").css("width").replace(/px/,"")- 0?$("#sidebar").css("width").replace(/px/,"")- 0:0;//此项目中除本身元素外其它元素所影响的left偏移数值
                        //var _otop = $("#header").height()?$("#header").height():0;//此项目中除本身元素外其它元素所影响的top偏移数值
                        difx = mouse.x - mouse.target.offset().left + daddy.offset().left + bLeft;
                        //difx = mouse.x - mouse.target.offset().left + _olef + bLeft;
                        dify = mouse.y - mouse.target.offset().top + daddy.offset().top + bTop;
                        //dify = mouse.y - mouse.target.offset().top + _otop + bTop;
