...
;(function(root, factory) {
    ...
    jet.reSetDate = function (obj,date,format) {
        ...
        var ndate = new Date(), narr = [], matArr = {YYYY:"FullYear",MM:"Month",DD:"Date",hh:"Hours",mm:"Minutes",ss:"Seconds"},
            dateY = date.YYYY == undefined ? ndate.getFullYear():date.YYYY,dateM = date.MM == undefined ? ndate.getMonth():date.MM,
        ...
        var reMonth = newDate.getMonth()+1;
    ...
    jedfn.initdates = function(){
        ...
        var formatDate = function (cls,boxcell) {
            ...
            // 是否限制选择最大时间为服务器时间
            var isServerMaxDate = (opts.isServerMaxDate == true || (opts.isServerMaxDate == undefined && config.isServerMaxDate)); 
            if (isServerMaxDate) {
                $.ajax({
                    ...
                    success: function(date) {
                        var endtime = $.timeStampDate(date.nowTime);
                        jet.maxDate = $.timeStampDate(endtime - (endtime % 10) - 20, "YYYY-MM-DD hh:mm:ss");
                    }
                });
            } else {
    ...
    jedfn.renderHtml = function(ys, ms, ds, opts,boxcls){
        ...
        var timeStr = function () {
            ...
            btnStr = '<span class="clear">'+clearTxt+'</span>' +
            		// '<span class="today">'+lang.today+'</span>' +
    				'<span class="setok">'+lang.yes+'</span>',
        ...
        if (testhh) {
            ...
            var rehms = jet.reMatch(minVal), vehms = [vals.hh, vals.mm, vals.ss - (vals.ss % 10)], hms = [];
            if (isrange) {
                if (that.getValue() == "") {
    ...
    jedfn.eachHms = function(opts,boxCell) {
        ...
        var isServerMaxDate = (opts.isServerMaxDate == true || (opts.isServerMaxDate == undefined && config.isServerMaxDate)); 
        if (isServerMaxDate) {
            // 设置最大时间后，选择其他日期时分秒不正确。
            var currday = boxCell.find(".daybox").find("td.actdate").attr("je-val");
            if (currday != maxTime[0]) {
                maxhms = [23, 59, 59];
            }
        }
        ...
        $.each(new Array(ranges ? 1 : 2),function (m) {
            ...
            $.each([24, 60, 60],function (i,lens) {
                ...
                for (var h = 0; h < lens; h++) {
                    ...
                    var tlis = $("<li/>", isSs ? {"data-i": jet.digit(h)} : null);
                    //判断限制时间范围的状态
                    if (opts.range != false){
                        if (isymdh){
                            if (m==0){
    ...
    jedfn.clickTime = function (opts,boxCell) {
        ...
        if(/\hh/.test(that.format)){
            ...
            var isServerMaxDate = (opts.isServerMaxDate == true || (opts.isServerMaxDate == undefined && config.isServerMaxDate));
            if(isServerMaxDate) {
            	var maxArr = jet.maxDate.split(" ");
                var maxHms = maxArr[1].split(":");
                var isCurrday = (maxArr[0] == boxCell.find(".daybox").find("td.actdate").attr("je-val"));
            }
            ...
            timeUl.on("click","li",function () {
                ...
                if (isServerMaxDate && isCurrday) {
                    if (ulidx == 0) {
                        if (maxHms[ulidx] == hmsval) {
                            $(timeUl[1]).find("li").eq(maxHms[1]).nextAll().addClass("disabled");
                            $(timeUl[2]).find("li").each(function() {
                            	var t = $(this);
                                if (t.data("i") == maxHms[2]) {
                                    t.nextAll().addClass("disabled");
                                }
                            });
                        } else {
                            $(timeUl[1]).find("li").removeClass("disabled");
                            $(timeUl[2]).find("li").removeClass("disabled");
                        }
                    } else if (ulidx == 1) {
                        if (maxHms[ulidx] == hmsval) {
                            $(timeUl[2]).find("li").each(function() {
