    ...
    e.prototype = {
        ...
        createDropdown: function() {
            var e = this.multiple ? " show-tick" : "",
                i = this.options.header ? '<h3 class="popover-title">' + this.options.header + '<button type="button" class="close" aria-hidden="true">&times;</button></h3>' : "",
                n = "<div class='btn-group bootstrap-select" + e + "'><button type='button' class='btn btn-sm form-control dropdown-toggle' data-toggle='dropdown'><div class='filter-option pull-left'></div>&nbsp;<div class='caret'></div></button><div class='dropdown-menu open'>" + i + "<ul class='dropdown-menu inner' role='menu'></ul></div></div>";
            return t(n)
        },
        createView: function() {
        ...
        createLi: function() {
            ...
            return this.$element.find("option").each(function() {
                var n = t(this),
                    s = n.attr("class") || "",
                    o = n.attr("style") || "",
                    a = n.data("content") ? n.data("content") : n.html(),
                    l = void 0 !== n.data("subtext") ? '<small class="muted">' + n.data("subtext") + "</small>" : "",
                    d = void 0 !== n.data("icon") ? '<i class="glyphicon ' + n.data("icon") + '"></i> ' : "";
                if ("" !== d && (n.is(":disabled") || n.parent().is(":disabled")) && (d = "<span>" + d + "</span>"), n.data("content") || (a = d + '<span class="text">' + a + l + "</span>"), e.options.hideDisabled && (n.is(":disabled") || n.parent().is(":disabled"))) i.push('<a style="min-height: 0; padding: 0"></a>');
                else if (n.parent().is("optgroup") && 1 != n.data("divider"))
                    if (0 == n.index()) {
                        var h = n.parent().attr("label"),
                            r = void 0 !== n.parent().data("subtext") ? '<small class="muted">' + n.parent().data("subtext") + "</small>" : "",
                            c = n.parent().data("icon") ? '<i class="' + n.parent().data("icon") + '"></i> ' : "";
                    ...
                    } else i.push(e.createA(a, "opt " + s, o));
                else i.push(1 == n.data("divider") ? '<div class="div-contain"><div class="divider"></div></div>' : 1 == t(this).data("hidden") ? "" : e.createA(a, s, o))
        ...
        render: function() {
            ...
            var i = this.$element.find("option:selected").map(function() {
                    var i, n = t(this),
                        s = n.data("icon") && e.options.showIcon ? '<i class="glyphicon ' + n.data("icon") + '"></i> ' : "";
                ...
                n = this.multiple ? i.join(", ") : i[0];
            if (e.multiple && e.options.selectedTextFormat.indexOf("count") > -1) {
                var s = e.options.selectedTextFormat.split(">"),
        ...
        liHeight: function() {
            ...
            t.appendTo("body");
            var e = t.addClass("open").find("> .dropdown-menu"),
        ...
        setSize: function() {
            var e, i, n, s = this,
                o = this.$menu,
                a = o.find(".inner"),
                l = (a.find("li > a"), this.$newElement.outerHeight()),
                d = this.$newElement.data("liHeight"),
                h = this.$newElement.data("headerHeight"),
                r = o.find("li .divider").outerHeight(!0),
                c = parseInt(o.css("padding-top")) + parseInt(o.css("padding-bottom")) + parseInt(o.css("border-top-width")) + parseInt(o.css("border-bottom-width")),
                p = this.options.hideDisabled ? ":not(.disabled)" : "",
                u = t(window),
                m = c + parseInt(o.css("margin-top")) + parseInt(o.css("margin-bottom")) + 2,
                f = function() {
            ...
            if (f(), this.options.header && o.css("padding-top", 0), "auto" == this.options.size) {
                ...
                var b = o.find("li" + p + " > *").filter(":not(.div-contain)").slice(0, this.options.size).last().parent().index(),
                    w = o.find("li").slice(0, b + 1).find(".div-contain").length;
        ...
        var s, o = arguments,
            a = this.each(function() {
                if (t(this).is("select")) {
                    var a = t(this),
                        l = a.data("selectpicker"),
                        d = "object" == typeof i && i;
                    if (l) {
                        if (d)
                            for (var h in d) l.options[h] = d[h]
                    } else a.data("selectpicker", l = new e(this, d, n)); if ("string" == typeof i) {
                        var r = i;
